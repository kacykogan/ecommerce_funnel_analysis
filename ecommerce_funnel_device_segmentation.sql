WITH funnel_events AS (
  -- 1. Pull relevant funnel events and device info
  SELECT
    user_pseudo_id,
    event_name,
    event_timestamp,
...
entry_device AS (
  -- 2. Identify the device used at the user's first funnel interaction
  SELECT
    user_pseudo_id,
    device_category,
    ROW_NUMBER() OVER (
...
user_device AS (
  -- 3. Keep only the first device per user
  SELECT
    user_pseudo_id,
    device_category
  FROM entry_device
...
ordered_events AS (
  -- 5. Order events per user and capture previous step
  SELECT
    user_pseudo_id,
    device_category,
    funnel_step,
...
valid_funnel AS (
  -- 6. Enforce proper funnel progression (no skipping)
  SELECT
    user_pseudo_id,
    device_category,
    funnel_step
...
final_funnel AS (
  -- 7. Deduplicate to one row per user per step
  SELECT DISTINCT
    user_pseudo_id,
    device_category,
    funnel_step,
...
-- 8. Aggregate overall funnel counts by device
SELECT
  device_category,
  funnel_step,
